-- ==========================================
-- LOGITRACK — SCRIPT COMPLETO DESDE CERO
-- ==========================================
...
BEGIN
    ...
    INTO v_tipo, v_origen, v_destino
    FROM movimientos
    WHERE id = NEW.movimiento_id;

    IF v_tipo = 'ENTRADA' THEN
        INSERT INTO bodega_producto (bodega_id, producto_id, stock)
        VALUES (v_destino, NEW.producto_id, NEW.cantidad)
        ON DUPLICATE KEY UPDATE stock = stock + NEW.cantidad;
    ELSEIF v_tipo = 'SALIDA' THEN
    UPDATE bodega_producto
    SET stock = stock - NEW.cantidad
    WHERE bodega_id = v_origen AND producto_id = NEW.producto_id;
...
BEGIN
    ...
    VALUES ('INSERT', 'Producto', NEW.id, NOW(), CURRENT_USER(), NULL,
...
BEGIN
    INSERT INTO auditorias (operacion, entidad, entidad_id, fecha_hora, usuario, valores_anteriores, valores_nuevos)
    VALUES ('UPDATE', 'Producto', NEW.id, NOW(), CURRENT_USER(),
...
BEGIN
    ...
    VALUES ('DELETE', 'Producto', OLD.id, NOW(), CURRENT_USER(),
            JSON_OBJECT('nombre', OLD.nombre, 'categoria_id', OLD.categoria_id, 'precio', OLD.precio), NULL);
END //
...
BEGIN
    INSERT INTO auditorias (operacion, entidad, entidad_id, fecha_hora, usuario, valores_anteriores, valores_nuevos)
    VALUES ('INSERT', 'Bodega', NEW.id, NOW(), CURRENT_USER(), NULL,
...
BEGIN
    ...
    VALUES ('DELETE', 'Bodega', OLD.id, NOW(), CURRENT_USER(),
            JSON_OBJECT('nombre', OLD.nombre, 'ubicacion', OLD.ubicacion, 'capacidad', OLD.capacidad, 'encargado_id', OLD.encargado_id), NULL);
END //
...
BEGIN
    INSERT INTO auditorias (operacion, entidad, entidad_id, fecha_hora, usuario, valores_anteriores, valores_nuevos)
    VALUES ('INSERT', 'Usuario', NEW.id, NOW(), CURRENT_USER(), NULL,
...
BEGIN
    ...
    VALUES ('DELETE', 'Usuario', OLD.id, NOW(), CURRENT_USER(),
            JSON_OBJECT('username', OLD.username, 'email', OLD.email, 'rol_id', OLD.rol_id), NULL);
END //
...
BEGIN
    INSERT INTO auditorias (operacion, entidad, entidad_id, fecha_hora, usuario, valores_anteriores, valores_nuevos)
    VALUES ('INSERT', 'Movimiento', NEW.id, NOW(), CURRENT_USER(), NULL,
...
BEGIN
    ...
    VALUES ('DELETE', 'Movimiento', OLD.id, NOW(), CURRENT_USER(),
            JSON_OBJECT('tipo', OLD.tipo, 'usuario_id', OLD.usuario_id, 'bodega_origen_id', OLD.bodega_origen_id, 'bodega_destino_id', OLD.bodega_destino_id), NULL);
END //
...
-- Roles (id 1 = ADMIN, id 2 = EMPLEADO)
...
-- Usuario ADMIN para el frontend → username: admin / password: 123456
INSERT INTO usuarios (id, username, email, password, rol_id) VALUES
                                                                 (1, 'admin', 'admin@logitrack.com', '$2b$10$TpsHT3bN2YmmypxPKXN78.pZ7Zah2onh.eAhrd.5uV0dFhTgSsVCK', 1),
                                                                 (2, 'empleado1', 'empleado1@logitrack.com', '$2b$10$TpsHT3bN2YmmypxPKXN78.pZ7Zah2onh.eAhrd.5uV0dFhTgSsVCK', 2);
...
INSERT INTO bodegas (id, nombre, ubicacion, capacidad, encargado_id) VALUES
                                                                         ...
                                                                         (3, 'Bodega Sur', 'Cali', 200, NULL);

-- Stock inicial
INSERT INTO bodega_producto (bodega_id, producto_id, stock) VALUES
                                                                (1, 1, 15), (1, 2, 3), (1, 3, 20), (1, 10, 18),
                                                                (2, 3, 8), (2, 5, 12), (2, 6, 7), (2, 7, 40),
                                                                (3, 8, 5), (3, 9, 25);

-- Movimientos (disparan triggers de stock y auditoría)
